...
php_script += "file_put_contents('deploy_ui_fix.zip', $zipData);\n"
php_script += "$zip = new ZipArchive;\n"
php_script += "if ($zip->open('deploy_ui_fix.zip') === TRUE) {\n"
php_script += "    $zip->extractTo('../');\n"
php_script += "    $zip->close();\n"
php_script += "    unlink('deploy_ui_fix.zip');\n"
php_script += "    \n"
...
php_script += "    }\n"
php_script += "    \n"
php_script += "    echo '<h1>UI Fixed!</h1>';\n"
php_script += "    echo '<p>Page Access Control added to invitations and header color fixed.</p>';\n"
php_script += "} else {\n"
php_script += "    echo '<h1>Error extracting zip</h1>';\n"
php_script += "}\n"
php_script += "?>"

with open('deploy_ui_fix.php', 'w') as f:
    f.write(php_script)
