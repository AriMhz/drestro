...
class NepaliCalendar {
    ...
    private function _is_in_range_nep($yy, $mm, $dd)
    {
        ...
        if ($dd < 1 || $dd > 32)
        {
            return 'Error! day value can be between 1-32 only';
        }
        return TRUE;
    }
    ...
    public function nep_to_eng($nyy, $nmm, $ndd)
    {
        $chk = $this->_is_in_range_nep($nyy, $nmm, $ndd);
        if ($chk !== TRUE) {
            return null;
        }
        ...
        if ($nyy == $def_nyy) {
            if ($nmm == $def_nmm) {
                ...
                $total_days = $this->_bs[0][$def_nmm] - $def_ndd;
                for ($m = $def_nmm + 1; $m < $nmm; $m++) {
                    $total_days += $this->_bs[0][$m];
                }
                $total_days += $ndd;
            } else {
                return null;
            ...
            // Days remaining in year 2000
            $total_days = $this->_bs[0][$def_nmm] - $def_ndd;
            for ($m = $def_nmm + 1; $m <= 12; $m++) {
                $total_days += $this->_bs[0][$m];
            }

            // Days in years 2001 to $nyy - 1
            for ($y = 2001; $y < $nyy; $y++) {
                $idx = $y - 2000;
                for ($m = 1; $m <= 12; $m++) {
                    $total_days += $this->_bs[$idx][$m];
                }
            }
        ...
        // Add total_days to 1944-01-01
        $base = \Carbon\Carbon::create(1944, 1, 1);
        $englishDate = $base->addDays($total_days);

        return [
            'year' => $englishDate->year,
            'month' => $englishDate->month,
            'date' => $englishDate->day,
            'formatted' => $englishDate->format('Y-m-d')
        ];
    }
    
    public static function formatSystemDate($carbonDate) {
        if (!$carbonDate) return '';
        $calType = strtoupper(current_restaurant()->date_calendar_type ?? 'AD');
        if ($calType === 'BS') {
            $nc = new self();
            ...
            if (isset($bs['month'])) {
                ...
                return $months[$bs['month'] - 1] . ' ' . $bs['date'] . ', ' . $bs['year'];
            }
        }
