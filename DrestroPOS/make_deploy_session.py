...
with zipfile.ZipFile('deploy.zip', 'w', zipfile.ZIP_DEFLATED) as zipf:
    ...
    for root, dirs, files in os.walk('app/Livewire/Auth'):
        for file in files:
            file_path = os.path.join(root, file)
            zipf.write(file_path, arcname=file_path.replace('\\', '/'))
...
php_script += "    echo '<h1>Session Auto-Lock & Logout Deployed!</h1>';\n"
php_script += "    echo '<p>PIN Lock and Logout button successfully installed.</p>';\n"
php_script += "} else {\n"
php_script += "    echo '<h1>Error extracting zip</h1>';\n"
php_script += "}\n"
php_script += "?>"

with open('deploy_session.php', 'w') as f:
    f.write(php_script)
