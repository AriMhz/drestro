<div class="h-full bg-slate-50 font-sans dark:bg-slate-900">
    <style>
        @media print {
    ...
    <div class="flex flex-col md:flex-row h-auto md:h-[calc(100vh-100px)]">
        
        <!-- Sidebar: Room Navigator -->
        <div class="{{ $selectedRoom ? 'hidden md:flex' : 'flex' }} w-full md:w-72 bg-white border-r border-slate-200 flex-col h-auto md:h-full shrink-0 dark:bg-slate-900 dark:border-slate-700">
            <div class="p-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center dark:border-slate-800 dark:bg-slate-900/50">
                <h2 class="text-sm font-bold text-slate-800 uppercase tracking-tight dark:text-slate-200">Active Guests</h2>
                <a href="{{ route('staff.cashier') }}?activeTab=hotel&statusFilter=completed" class="text-[10px] font-bold text-indigo-600 hover:text-indigo-800 bg-indigo-50 hover:bg-indigo-100 px-3 py-1.5 rounded-lg transition-all flex items-center gap-1 dark:bg-indigo-900/20" title="View Past Stays">
            ...
            <div class="flex-1 overflow-visible md:overflow-y-auto p-3 space-y-2">
                ...
                <button wire:click="selectRoom({{ $room->id }})" class="w-full p-4 rounded-xl text-left transition-all {{ $selectedRoomId == $room->id ? 'bg-indigo-600 text-white shadow-lg dark:bg-indigo-500 dark:text-white' : 'bg-white border border-slate-200 text-slate-600 hover:border-indigo-300 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-700 dark:hover:text-slate-200' }}">
                    <div class="flex justify-between items-center">
                        <span class="text-lg font-bold">Room {{ $room->name }}</span>
                        <div class="w-2 h-2 rounded-full {{ $selectedRoomId == $room->id ? 'bg-white' : 'bg-emerald-500' }} dark:bg-slate-900"></div>
                    </div>
        ...
        <!-- Main Workspace -->
        <div class="{{ $selectedRoom ? 'block' : '' }} flex-1 overflow-visible md:overflow-y-auto p-4 md:p-8 relative">
            @if($selectedRoom)
                <!-- Guest Header -->
                <div class="bg-white rounded-2xl p-4 md:p-6 shadow-sm border border-slate-200 mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 md:gap-0 dark:bg-slate-900 dark:border-slate-700">
                    <div class="flex items-center gap-3 md:gap-5 w-full md:w-auto">
                        <button wire:click="$set('selectedRoom', null)" class="md:hidden p-2 -ml-2 bg-slate-100 rounded-lg text-slate-600 hover:bg-slate-200 shrink-0 dark:text-slate-400 dark:bg-slate-800">
                        ...
                        <div>
                            <h2 class="text-xl font-bold text-slate-900 dark:text-slate-100">{{ $selectedRoom->guest_name }}</h2>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 md:gap-8" id="pdf-content">
                    ...
                    <div class="lg:col-span-8 space-y-8">
                        <!-- Stay Charges -->
                        <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 dark:bg-slate-900 dark:border-slate-700">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
                                <div class="bg-slate-50 p-4 rounded-xl border border-slate-100 text-center dark:border-slate-800 dark:bg-slate-900">
                                    <p class="text-[10px] font-bold text-slate-400 uppercase mb-1">Rate</p>
                                    <p class="text-lg font-bold text-slate-900 dark:text-slate-100">Rs. {{ number_format($selectedRoom->room_rate, 0) }}</p>
                                </div>
                                <div class="bg-slate-50 p-4 rounded-xl border border-slate-100 text-center dark:border-slate-800 dark:bg-slate-900">
                                    <p class="text-[10px] font-bold text-slate-400 uppercase mb-1">Duration</p>
                                    <p class="text-lg font-bold text-slate-900 dark:text-slate-100">{{ max(1, \Carbon\Carbon::parse($selectedRoom->check_in_at)->diffInDays(now())) + $manualDays }} Days</p>
                                </div>
                                <div class="bg-indigo-50 p-4 rounded-xl border border-indigo-100 text-center dark:border-indigo-800/50 dark:bg-indigo-900/20">
                        ...
                        <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 dark:bg-slate-900 dark:border-slate-700">
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="p-4 bg-slate-50 rounded-xl flex justify-between items-center border border-slate-100 dark:border-slate-800 dark:bg-slate-900">
                                    <div>
                                        <p class="text-xs font-bold text-slate-800 uppercase tracking-tight dark:text-slate-200">Order #{{ $order->order_number }}</p>
                                        <p class="text-[10px] text-slate-500 dark:text-slate-400">{{ $order->created_at->format('h:i A') }} • {{ $order->items->count() }} items</p>
                                    </div>
                                    <span class="text-sm font-bold text-slate-900 dark:text-slate-100">Rs. {{ number_format($order->total_amount, 0) }}</span>
                                </div>
                                @empty
                    ...
                    <div class="lg:col-span-4">
                        <div class="bg-slate-900 rounded-2xl p-6 shadow-lg text-white">
                            ...
                            <button wire:click="openPaymentModal" class="w-full py-4 bg-white text-slate-900 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-emerald-500 hover:text-white transition-all dark:bg-slate-900 dark:text-slate-100">
                                Confirm Checkout
                            </button>
                        </div>
                    </div>
            ...
            @else
                ...
                <div class="h-auto md:h-full flex flex-col overflow-visible md:overflow-hidden bg-slate-50 print:bg-white print:overflow-visible dark:bg-slate-900">
                    <!-- Top capsule header (print-hidden) -->
                    <div class="p-6 border-b border-slate-100 flex flex-wrap items-center justify-between gap-4 bg-white shadow-sm print-hidden rounded-2xl mb-6 dark:border-slate-800 dark:bg-slate-900">
                        <div>
                            ...
                            <p class="text-xs text-slate-500 font-medium dark:text-slate-400">Occupancy status and stay settlements record.</p>
                        </div>
                        
                        <div class="flex flex-col sm:flex-row flex-wrap items-stretch gap-4 bg-white rounded-2xl border border-slate-200 p-4 shadow-sm print-hidden w-full xl:w-auto mt-2 xl:mt-0 dark:bg-slate-900 dark:border-slate-700">
                                    ...
                                    <div class="flex justify-between items-center pb-2 border-b border-slate-100 dark:border-slate-800">
                                        ...
                                        @if(auth()->user()->role === 'super_admin')
                                            <div class="flex items-center bg-slate-100 border border-slate-200 p-0.5 rounded-lg text-[9px] font-black uppercase tracking-widest select-none shrink-0 dark:border-slate-700 dark:bg-slate-800">
                                                <a href="{{ route('toggle-calendar', 'ad') }}" class="px-2 py-0.5 rounded transition-all {{ $currentCal === 'AD' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-800' }} dark:text-slate-400">AD</a>
                                                <a href="{{ route('toggle-calendar', 'bs') }}" class="px-2 py-0.5 rounded transition-all {{ $currentCal === 'BS' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-800' }} dark:text-slate-400">BS</a>
                                            </div>
                                        @else
                            ...
                            <div class="flex flex-col gap-1.5 justify-between">
                                ...
                                <div class="flex flex-wrap items-center gap-2 shrink-0">
                                    <button onclick="window.print()" class="px-4 py-1.5 rounded-xl text-xs font-bold text-slate-700 bg-slate-100 hover:bg-slate-200 transition-all flex items-center gap-2 border border-slate-200 shadow-sm dark:text-slate-300 dark:border-slate-700 dark:bg-slate-800" title="Print Report">
                                        <svg class="w-3.5 h-3.5 text-slate-500 dark:text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"></path></svg>
                                        Print
                                    </button>
                                    <button onclick="downloadPDF('Hotel_Cashier_Report.pdf')" class="px-4 py-1.5 rounded-xl text-xs font-bold text-indigo-700 bg-indigo-50 border border-indigo-100 hover:bg-indigo-100 transition-all flex items-center gap-2 shadow-sm dark:border-indigo-800/50 dark:text-indigo-400 dark:bg-indigo-900/20" title="Save Report as PDF">
                    ...
                    <div class="flex-1 overflow-y-auto p-6 sm:p-8 space-y-8 print:overflow-visible print:p-0" id="hotel-sales-report-content">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <!-- Card 1: Occupancy -->
                            <div class="bg-gradient-to-br from-indigo-50 to-indigo-100/50 border border-indigo-100 p-5 rounded-2xl shadow-sm flex flex-col justify-between dark:border-indigo-800/50 dark:to-indigo-900/10 dark:from-indigo-900/40">
                                <span class="text-[10px] font-black text-indigo-800 uppercase tracking-wider dark:text-indigo-300">Occupancy</span>
                                <div class="mt-2">
                                    <span class="text-2xl font-black text-slate-900 dark:text-slate-100">{{ round(($stats['total_occupied'] / max(1, $stats['total_rooms'])) * 100) }}%</span>
                                    <p class="text-[10px] text-slate-500 font-bold mt-1 dark:text-slate-400">{{ $stats['total_occupied'] }}/{{ $stats['total_rooms'] }} Rooms occupied</p>
                                </div>
                            </div>
                            
                            <!-- Card 2: Revenue -->
                            <div class="bg-gradient-to-br from-emerald-50 to-emerald-100/50 border border-emerald-100 p-5 rounded-2xl shadow-sm flex flex-col justify-between dark:to-emerald-900/10 dark:from-emerald-900/40 dark:border-emerald-800/50">
                                <span class="text-[10px] font-black text-emerald-800 uppercase tracking-wider dark:text-emerald-300">Stay Revenue in Selected Period</span>
                                <div class="mt-2">
                                    <span class="text-2xl font-black text-slate-900 dark:text-slate-100">Rs. {{ number_format($stats['revenue_today'], 0) }}</span>
                                    <p class="text-[10px] text-slate-500 font-bold mt-1 dark:text-slate-400">From hotel checkouts in range</p>
                                </div>
                            </div>

                            <!-- Card 3: Available -->
                            <div class="bg-gradient-to-br from-slate-50 to-slate-100/50 border border-slate-200 p-5 rounded-2xl shadow-sm flex flex-col justify-between dark:border-slate-700 dark:to-slate-900/20 dark:from-slate-900/60">
                                <span class="text-[10px] font-black text-slate-700 uppercase tracking-wider dark:text-slate-300">Available Rooms</span>
                                <div class="mt-2">
                                    <span class="text-2xl font-black text-slate-900 dark:text-slate-100">{{ $stats['total_rooms'] - $stats['total_occupied'] }} Rooms</span>
                                    <p class="text-[10px] text-slate-500 font-bold mt-1 dark:text-slate-400">Ready for check-in</p>
                                </div>
                            </div>
                        </div>

                        <!-- Today's Checked-out Stay Orders Archive -->
                        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 dark:bg-slate-900 dark:border-slate-700">
                            ...
                            <div class="overflow-x-auto">
                                <table class="w-full text-left border-collapse">
                                    <thead>
                                        <tr class="border-b border-slate-200 bg-slate-50/50 dark:border-slate-700 dark:bg-slate-900/50">
                                        ...
                                        @forelse($this->todayCheckouts as $checkout)
                                            ...
                                            <tr class="hover:bg-slate-50/50 transition-colors">
                                                <td class="py-3.5 px-4">
                                                    <span class="text-xs font-bold text-slate-900 dark:text-slate-100">Room {{ $checkout->table->name }}</span>
                                                </td>
                                                <td class="py-3.5 px-4">
                                                    <div class="flex flex-col">
                                                        <span class="text-xs font-bold text-slate-700 dark:text-slate-300">{{ $guestName ?: 'Guest' }}</span>
                                                        @if($guestPhone)
                                                ...
                                                <td class="py-3.5 px-4 text-slate-500 text-xs font-medium dark:text-slate-400">
                                                    {{ $checkout->updated_at->format('M d, Y') }} at {{ $checkout->updated_at->format('h:i A') }}
                                                </td>
                                                <td class="py-3.5 px-4">
                                                    <span class="px-2.5 py-1 bg-slate-100 text-slate-600 rounded-lg text-[9px] font-black uppercase tracking-widest dark:text-slate-400 dark:bg-slate-800">
                                                        {{ $checkout->invoice->payment_method ?? 'CASH' }}
                                                    </span>
                                                </td>
                                                <td class="py-3.5 px-4 text-right">
                                                    <span class="text-xs font-black text-slate-900 dark:text-slate-100">Rs. {{ number_format($checkout->invoice->grand_total ?? $checkout->total_amount, 0) }}</span>
                                                </td>
                                            </tr>
                                        @empty
                                            <tr>
    ...
    <div class="fixed inset-0 z-[100] bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-white rounded-3xl overflow-hidden w-full max-w-lg shadow-2xl flex flex-col max-h-[85vh] sm:max-h-[90vh] dark:bg-slate-900">
            <div class="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 dark:border-slate-800 dark:bg-slate-900/50">
                <h3 class="font-bold text-xl text-slate-800 dark:text-slate-200">Final Stay Settlement</h3>
            ...
            <div class="p-8 overflow-y-auto flex-1 custom-scrollbar">
                <div class="text-center mb-8">
                    <div class="text-sm text-slate-500 font-bold uppercase tracking-wider mb-2 dark:text-slate-400">Grand Total Payable</div>
                ...
                <div class="mb-8 p-6 bg-slate-50 rounded-2xl border border-slate-200 dark:bg-slate-900 dark:border-slate-700">
                    <label class="block text-sm font-black text-slate-700 mb-3 uppercase tracking-widest dark:text-slate-300">Apply Discount (Rs.)</label>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                            <span class="text-slate-400 font-bold">Rs.</span>
                        </div>
                        <input type="number" wire:model.blur="discount" class="w-full pl-12 pr-4 py-4 bg-white border-2 border-indigo-100 rounded-xl text-xl font-black text-indigo-600 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 outline-none transition-all dark:bg-slate-900 dark:border-indigo-800/50" placeholder="0.00">
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <label class="cursor-pointer">
                            ...
                            <div :class="$wire.paymentMethod === 'cash' ? 'border-indigo-500 bg-indigo-50 shadow-sm' : 'border-slate-200 hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-800/50'" class="p-4 rounded-xl border-2 transition-all flex items-center gap-3">
                                <div class="w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-sm dark:bg-slate-900">💵</div>
                                <span :class="$wire.paymentMethod === 'cash' ? 'text-slate-900 dark:text-slate-900' : 'text-slate-700 dark:text-slate-300'" class="font-bold">Cash</span>
                            </div>
                        </label>
                        <label class="cursor-pointer">
                            <input type="radio" wire:model.live="paymentMethod" value="card" class="sr-only">
                            <div :class="$wire.paymentMethod === 'card' ? 'border-indigo-500 bg-indigo-50 shadow-sm' : 'border-slate-200 hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-800/50'" class="p-4 rounded-xl border-2 transition-all flex items-center gap-3">
                                <div class="w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-sm dark:bg-slate-900">💳</div>
                                <span :class="$wire.paymentMethod === 'card' ? 'text-slate-900 dark:text-slate-900' : 'text-slate-700 dark:text-slate-300'" class="font-bold">Card</span>
                            </div>
                        </label>
                        <label class="cursor-pointer col-span-2">
                            <input type="radio" wire:model.live="paymentMethod" value="online" class="sr-only">
                            <div :class="$wire.paymentMethod === 'online' ? 'border-indigo-500 bg-indigo-50 shadow-sm' : 'border-slate-200 hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-800/50'" class="p-4 rounded-xl border-2 transition-all flex items-center gap-3">
                                <div class="w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-sm dark:bg-slate-900">📱</div>
                                <span :class="$wire.paymentMethod === 'online' ? 'text-slate-900 dark:text-slate-900' : 'text-slate-700 dark:text-slate-300'" class="font-bold">Online Wallet / Transfer</span>
                            </div>
                        </label>
                        <label class="cursor-pointer col-span-2">
                            <input type="radio" wire:model.live="paymentMethod" value="split" class="sr-only">
                            <div :class="$wire.paymentMethod === 'split' ? 'border-indigo-500 bg-indigo-50 shadow-sm' : 'border-slate-200 hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-800/50'" class="p-4 rounded-xl border-2 transition-all flex items-center gap-3">
                                <div class="w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-sm dark:bg-slate-900">⚖️</div>
                                <span :class="$wire.paymentMethod === 'split' ? 'text-slate-900 dark:text-slate-900' : 'text-slate-700 dark:text-slate-300'" class="font-bold">Split Payment (Cash & Online)</span>
                            </div>
                        </label>
                    </div>
                    ...
                    @if($paymentMethod === 'online')
                        <div class="mt-4 p-4 bg-slate-50 rounded-xl border border-slate-200 dark:bg-slate-900 dark:border-slate-700 space-y-4">
                            <div>
                                ...
                                <select wire:model="paymentProvider" class="w-full px-4 py-3 rounded-lg border border-slate-300 focus:ring-indigo-500 focus:border-indigo-500 dark:border-slate-650 dark:bg-slate-900 dark:text-slate-100">
                    ...
                    @if($paymentMethod === 'split')
                        <div class="mt-4 p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-4 dark:bg-slate-900 dark:border-slate-700">
                            <div>
                                ...
                                <input type="number" wire:model.live.debounce.500ms="splitCashAmount" class="w-full px-4 py-3 rounded-lg border border-slate-300 focus:ring-indigo-500 focus:border-indigo-500 font-bold dark:border-slate-650 dark:bg-slate-900 dark:text-slate-100" placeholder="Enter Cash Paid">
                            </div>
                            <div class="flex justify-between items-center text-sm font-bold p-3 bg-indigo-50 border border-indigo-100 rounded-lg text-indigo-800 dark:border-indigo-800/50 dark:text-indigo-300 dark:bg-indigo-900/20">
