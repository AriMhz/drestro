<div class="h-full flex flex-col -m-4 sm:-m-6 lg:-m-8">
    ...
    <div class="flex-1 flex flex-col md:flex-row overflow-y-auto md:overflow-hidden bg-slate-50">
        
        <!-- Left Side: Active Orders List -->
        <div class="{{ $selectedOrder ? 'hidden md:flex' : 'flex' }} w-full md:w-1/3 lg:w-1/4 bg-white border-b md:border-b-0 md:border-r border-slate-200 flex-col h-[50vh] md:h-full shrink-0 z-10 shadow-[4px_0_24px_rgba(0,0,0,0.02)]" wire:poll.10s>
            <div class="p-4 border-b border-slate-100 bg-slate-50/50 sticky top-0">
                <div class="flex bg-slate-200/50 p-1 rounded-xl mb-2">
                    <button wire:click="$set('activeTab', 'restaurant')" class="flex-1 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all {{ $activeTab === 'restaurant' ? 'bg-white text-emerald-600 shadow-sm' : 'text-slate-500 hover:text-slate-700' }}">
                        Restaurant
                    </button>
                    <button wire:click="$set('activeTab', 'hotel')" class="flex-1 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all {{ $activeTab === 'hotel' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700' }}">
                        Hotel
                    </button>
                </div>
                <!-- Status Filter Tabs -->
                <div class="flex bg-slate-100/80 p-0.5 rounded-lg mb-2">
                    <button wire:click="$set('statusFilter', 'active')" class="flex-1 py-1.5 text-[9px] font-bold uppercase tracking-wider rounded-md transition-all {{ $statusFilter === 'active' ? 'bg-white text-amber-600 shadow-sm' : 'text-slate-400 hover:text-slate-600' }}">
                        Active
                    </button>
                    <button wire:click="$set('statusFilter', 'completed')" class="flex-1 py-1.5 text-[9px] font-bold uppercase tracking-wider rounded-md transition-all {{ $statusFilter === 'completed' ? 'bg-white text-emerald-600 shadow-sm' : 'text-slate-400 hover:text-slate-600' }}">
                        Done
                    </button>
                    <button wire:click="$set('statusFilter', 'cancelled')" class="flex-1 py-1.5 text-[9px] font-bold uppercase tracking-wider rounded-md transition-all {{ $statusFilter === 'cancelled' ? 'bg-white text-red-600 shadow-sm' : 'text-slate-400 hover:text-slate-600' }}">
                        Cancelled
                    </button>
                    <button wire:click="$set('statusFilter', 'all')" class="flex-1 py-1.5 text-[9px] font-bold uppercase tracking-wider rounded-md transition-all {{ $statusFilter === 'all' ? 'bg-white text-slate-700 shadow-sm' : 'text-slate-400 hover:text-slate-600' }}">
                        All
                    </button>
                </div>
            ...
            <div class="flex-1 overflow-y-auto p-3 space-y-2">
                @forelse($this->activeOrders as $order)
                    <button wire:click="selectOrder({{ $order->id }})" class="w-full text-left p-4 rounded-xl border {{ $selectedOrder && $selectedOrder->id == $order->id ? 'border-emerald-500 bg-emerald-50 shadow-sm' : 'border-slate-100 hover:border-slate-300 hover:bg-slate-50' }} transition-all relative group">
                        <div class="flex justify-between items-start mb-2">
                            <span class="font-bold {{ $selectedOrder && $selectedOrder->id == $order->id ? 'text-emerald-800' : 'text-slate-800' }}">
                        ...
                        <div class="flex justify-between items-end">
                            <span class="text-xs text-slate-500 font-medium">#{{ substr($order->order_number, -4) }}</span>
                            <span class="font-bold {{ $selectedOrder && $selectedOrder->id == $order->id ? 'text-emerald-700' : 'text-slate-900' }}">
                                @php
                ...
                @empty
                    <div class="py-10 text-center">
                        ...
                        <p class="text-sm font-medium text-slate-500">No active orders</p>
                    </div>
                @endforelse
            </div>
        </div>
        ...
        <div class="{{ $selectedOrder ? 'flex min-h-[50vh]' : 'flex min-h-[50vh]' }} flex-1 bg-slate-50/50 flex-col overflow-visible md:overflow-hidden relative">
            @if($selectedOrder)
                <!-- Invoice Header -->
                <div class="bg-white border-b border-slate-200 px-8 py-5 flex justify-between items-center z-10 shadow-sm">
                    <div class="flex items-start md:items-center gap-3">
                        <button wire:click="$set('selectedOrder', null)" class="md:hidden mt-1 p-2 -ml-2 bg-slate-100 rounded-lg text-slate-600 hover:bg-slate-200">
                    ...
                    <div class="flex gap-2">
                        ...
                        <button wire:click="printThermalReceiptDirect" wire:loading.attr="disabled" class="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold transition-all flex items-center gap-2 shadow-md shadow-emerald-600/10 active:scale-95 disabled:opacity-50" title="Print raw receipt directly to Thermal Network Printer">
                ...
                @if (session()->has('success'))
                    <div class="bg-emerald-50 border-b border-emerald-200 px-8 py-3 flex items-center gap-3 text-emerald-700 text-xs font-bold transition-all">
                        <svg class="w-4 h-4 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                        <span>{{ session('success') }}</span>
                    </div>
                @endif
                @if (session()->has('error'))
                    <div class="bg-rose-50 border-b border-rose-200 px-8 py-3 flex items-center gap-3 text-rose-700 text-xs font-bold transition-all">
                ...
                <div class="flex-1 overflow-y-auto p-8 flex justify-center printable-area" id="pdf-content">
                    <div class="bg-white max-w-lg w-full rounded-2xl shadow-sm border border-slate-200 p-8 h-fit" id="invoice">
                        <!-- Restaurant Header -->
                        <div class="text-center mb-6 pb-6 border-b border-dashed border-slate-300">
                            <h1 class="text-2xl font-black text-slate-900 tracking-tight">{{ $restaurant->name }}</h1>
                            @if($restaurant->address)
                                <p class="text-slate-500 text-xs mt-1">{{ $restaurant->address }}</p>
                            @endif
                            @if($restaurant->phone)
                                <p class="text-slate-500 text-xs">Phone: {{ $restaurant->phone }}</p>
                            @endif
                            @if($restaurant->pan_number)
                                <p class="text-slate-500 text-xs font-semibold">PAN: {{ $restaurant->pan_number }}</p>
                            @endif
                            <p class="text-slate-400 text-xs mt-2">--- INVOICE ---</p>
                        </div>

                        <!-- Order Info -->
                        <div class="mb-4 pb-4 border-b border-dashed border-slate-200 text-xs text-slate-600 space-y-1">
                            ...
                            <tbody class="text-sm">
                                @foreach($selectedOrder->items as $item)
                                    <tr class="border-b border-slate-50">
                                        <td class="py-2 text-slate-800 font-medium">{{ $item->menuItem->name }}</td>
                                        <td class="py-2 font-bold text-slate-700 text-center">{{ $item->quantity }}</td>
                                        <td class="py-2 text-right font-bold text-slate-700">{{ number_format($item->subtotal, 0) }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                        ...
                        <!-- Totals -->
                        <div class="border-t border-dashed border-slate-300 pt-3 space-y-1 text-sm">
                            <div class="flex justify-between text-slate-600">
                                <span>Subtotal</span>
                                <span>{{ $restaurant->currency }} {{ number_format($selectedOrder->total_amount, 0) }}</span>
                            </div>
                            @if($restaurant->tax_percent > 0)
                            <div class="flex justify-between text-slate-600">
                                <span>Tax ({{ $restaurant->tax_percent }}%)</span>
                                <span>{{ $restaurant->currency }} {{ number_format($selectedOrder->total_amount * $restaurant->tax_percent / 100, 0) }}</span>
                            </div>
                            @endif
                            @if($restaurant->service_charge_percent > 0)
                            <div class="flex justify-between text-slate-600">
                                <span>Service Charge ({{ $restaurant->service_charge_percent }}%)</span>
                                <span>{{ $restaurant->currency }} {{ number_format($selectedOrder->total_amount * $restaurant->service_charge_percent / 100, 0) }}</span>
                            </div>
                            @endif
                            @php
                                $taxAmt = $selectedOrder->total_amount * ($restaurant->tax_percent ?? 0) / 100;
                                $scAmt = $selectedOrder->total_amount * ($restaurant->service_charge_percent ?? 0) / 100;
                                $invoiceDiscount = $selectedOrder->invoice ? $selectedOrder->invoice->discount : (float)$discount;
                                $grandTotal = ($selectedOrder->total_amount + $taxAmt + $scAmt) - $invoiceDiscount;
                            @endphp
                            ...
                            <div class="flex justify-between items-center pt-3 border-t border-slate-200">
                                ...
                                <span class="text-2xl font-black text-emerald-600">{{ $restaurant->currency }} {{ number_format($grandTotal > 0 ? $grandTotal : 0, 0) }}</span>
                            </div>
                        </div>

                        <!-- Payment Info -->
                        @if($selectedOrder->invoice && $selectedOrder->invoice->payment_status === 'paid')
                        <div class="mt-4 pt-3 border-t border-dashed border-slate-300 text-center text-xs text-slate-500">
                            <p class="font-bold">PAID via {{ ucfirst($selectedOrder->invoice->payment_provider ?: $selectedOrder->invoice->payment_method) }}</p>
                        </div>
                        @endif
                        
                        <div class="mt-4 pt-3 border-t border-dashed border-slate-300 text-center text-xs text-slate-500 font-medium">
                ...
                <div class="bg-white border-t border-slate-200 p-6 flex justify-end gap-4 shadow-[0_-4px_24px_rgba(0,0,0,0.02)]">
                    <div class="mr-auto">
                        @php
                            $uiTax = $selectedOrder->total_amount * ($restaurant->tax_percent ?? 0) / 100;
                            $uiSc = $selectedOrder->total_amount * ($restaurant->service_charge_percent ?? 0) / 100;
                            $uiTotal = ($selectedOrder->total_amount + $uiTax + $uiSc) - (float)$discount;
                        @endphp
                        <div class="text-3xl font-black text-slate-900">Rs. {{ number_format($uiTotal > 0 ? $uiTotal : 0, 0) }}</div>
                    </div>
                    @if($selectedOrder->invoice && $selectedOrder->invoice->payment_status === 'paid')
                        <div class="flex items-center gap-3">
                            <span class="px-4 py-2 bg-emerald-100 text-emerald-800 font-bold rounded-lg border border-emerald-200">
                    ...
                    @elseif($selectedOrder->status === 'cancelled')
                        <span class="px-5 py-2.5 bg-red-100 text-red-800 font-bold rounded-xl border border-red-200 flex items-center gap-2">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                            Cancelled
                        </span>
                    @else
                        <button wire:click="cancelOrder" onclick="confirm('Cancel this order?') || event.stopImmediatePropagation()" class="px-6 py-3 bg-red-50 hover:bg-red-100 text-red-600 font-bold rounded-xl border border-red-200 active:scale-95 transition-all text-lg flex items-center">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                            Cancel
                        </button>
                        <button wire:click="openPaymentModal" class="px-8 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl shadow-lg shadow-emerald-600/20 active:scale-95 transition-all text-lg flex items-center">
            ...
            @else
                ...
                <div class="h-full flex flex-col overflow-hidden bg-slate-50 print:bg-white print:overflow-visible" id="pdf-content">
                    <!-- Top capsule header (print-hidden) -->
                    <div class="p-6 border-b border-slate-100 flex flex-wrap items-center justify-between gap-4 bg-white shadow-sm print-hidden">
                        <div>
                            ...
                            <p class="text-xs text-slate-500 font-medium">Real-time overview of your cash counter sales.</p>
                        </div>
                        
                        <div class="flex flex-wrap items-stretch gap-4 bg-white rounded-2xl border border-slate-200 p-4 shadow-sm print-hidden w-full xl:w-auto mt-2 xl:mt-0">
                            <!-- Section 1: Presets -->
                                ...
                                <div x-show="pickerOpen" x-cloak 
                                    ...
                                    <div class="flex justify-between items-center pb-2 border-b border-slate-100">
                                        ...
                                        @if(auth()->user()->role === 'super_admin')
                                            <div class="flex items-center bg-slate-100 border border-slate-200 p-0.5 rounded-lg text-[9px] font-black uppercase tracking-widest select-none shrink-0">
                                                <a href="{{ route('toggle-calendar', 'ad') }}" class="px-2 py-0.5 rounded transition-all {{ $currentCal === 'AD' ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-800' }}">AD</a>
                                                <a href="{{ route('toggle-calendar', 'bs') }}" class="px-2 py-0.5 rounded transition-all {{ $currentCal === 'BS' ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-800' }}">BS</a>
                                            </div>
                                        @else
                                    ...
                                    <div class="pt-2 border-t border-slate-100 flex flex-col gap-1 text-[10px] select-none">
                                        <div class="flex justify-between items-center text-[9px] text-slate-400 font-bold uppercase">
                                            ...
                                            <span class="text-emerald-600 font-black uppercase tracking-wider" x-text="pickingField === 'start' ? 'Selecting From' : 'Selecting To'"></span>
                                        </div>
                            ...
                            <div class="flex flex-col gap-1.5 justify-between">
                                ...
                                <div class="flex flex-wrap items-center gap-2 shrink-0">
                                    <button onclick="printDailySummary()" class="px-4 py-1.5 rounded-xl text-xs font-bold text-slate-700 bg-slate-100 hover:bg-slate-200 transition-all flex items-center gap-2 border border-slate-200 shadow-sm" title="Print Daily Summary">
                                        <svg class="w-3.5 h-3.5 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"></path></svg>
                                        Print
                                    </button>
                                    <button onclick="printDailySummary()" class="px-4 py-1.5 rounded-xl text-xs font-bold text-indigo-700 bg-indigo-50 border border-indigo-100 hover:bg-indigo-100 transition-all flex items-center gap-2 shadow-sm" title="Save Daily Summary as PDF">
                    ...
                    <div class="flex-1 overflow-y-auto p-6 sm:p-8 space-y-8 print:overflow-visible print:p-0" id="daily-sales-report-content">
                        <!-- Printable Header -->
                        <div class="hidden print:block text-center mb-8">
                            <h1 class="text-3xl font-black uppercase text-slate-900 tracking-tight">{{ $restaurant->name ?? 'DrestroPOS' }}</h1>
                            <p class="text-sm font-bold text-slate-500 uppercase tracking-widest mt-1">Sales Summary & Record Report</p>
                        ...
                        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                            ...
                            <div class="bg-gradient-to-br from-emerald-50 to-emerald-100/50 border border-emerald-100 p-5 rounded-[1.5rem] shadow-sm flex flex-col justify-between">
                                <span class="text-[10px] font-black text-emerald-800 uppercase tracking-wider">Total Sales</span>
                                <div class="mt-2">
                                    <span class="text-2xl font-black text-emerald-950">Rs. {{ number_format($this->dailySummaryData['grand_total'], 0) }}</span>
                                    <p class="text-[9px] text-emerald-700 font-bold uppercase tracking-wider mt-1">{{ $this->dailySummaryData['count'] }} Orders Completed</p>
                                </div>
                            </div>
                            
                            <!-- Card 2: Cash Collection -->
                            <div class="bg-gradient-to-br from-slate-50 to-slate-100/50 border border-slate-200/60 p-5 rounded-[1.5rem] shadow-sm flex flex-col justify-between">
                                <span class="text-[10px] font-black text-slate-500 uppercase tracking-wider">Paid via Cash</span>
                                <div class="mt-2">
                                    <span class="text-2xl font-black text-slate-800">Rs. {{ number_format($this->dailySummaryData['cash_total'], 0) }}</span>
                                    <p class="text-[9px] text-slate-400 font-bold uppercase tracking-wider mt-1">Cash in Drawer</p>
                                </div>
                            </div>

                            <!-- Card 3: Card Collection -->
                            <div class="bg-gradient-to-br from-indigo-50 to-indigo-100/50 border border-indigo-100 p-5 rounded-[1.5rem] shadow-sm flex flex-col justify-between">
                                <span class="text-[10px] font-black text-indigo-700 uppercase tracking-wider">Paid via Card</span>
                                <div class="mt-2">
                                    <span class="text-2xl font-black text-indigo-950">Rs. {{ number_format($this->dailySummaryData['card_total'], 0) }}</span>
                                    <p class="text-[9px] text-indigo-700 font-bold uppercase tracking-wider mt-1">Bank Settlements</p>
                                </div>
                            </div>

                            <!-- Card 4: Online Collection -->
                            <div class="bg-gradient-to-br from-amber-50 to-amber-100/50 border border-amber-100 p-5 rounded-[1.5rem] shadow-sm flex flex-col justify-between">
                                <span class="text-[10px] font-black text-amber-700 uppercase tracking-wider">Online / Wallet</span>
                                <div class="mt-2">
                                    <span class="text-2xl font-black text-amber-950">Rs. {{ number_format($this->dailySummaryData['online_total'], 0) }}</span>
                                    <p class="text-[9px] text-amber-700 font-bold uppercase tracking-wider mt-1">Fonepay & Online Wallets</p>
                                </div>
                            </div>
                        </div>

                        <!-- Financial breakdown table -->
                        <div class="bg-white border border-slate-100 rounded-[1.5rem] p-6 shadow-sm">
                            <h4 class="text-xs font-black text-slate-800 uppercase tracking-wider mb-4">Financial Breakdown</h4>
                            <div class="space-y-3.5">
                                <div class="flex justify-between text-sm">
                                    <span class="text-slate-500 font-medium">Subtotal Revenue</span>
                                    <span class="font-bold text-slate-800">Rs. {{ number_format($this->dailySummaryData['subtotal'], 0) }}</span>
                                </div>
                                <div class="flex justify-between text-sm">
                                    <span class="text-slate-500 font-medium">VAT & Taxes</span>
                                    <span class="font-bold text-slate-800">Rs. {{ number_format($this->dailySummaryData['tax'], 0) }}</span>
                                </div>
                                <div class="flex justify-between text-sm">
                                    <span class="text-slate-500 font-medium">Total Discounts Given</span>
                                    <span class="font-bold text-red-600">- Rs. {{ number_format($this->dailySummaryData['discount'], 0) }}</span>
                                </div>
                                <div class="h-px bg-slate-100"></div>
                                <div class="flex justify-between text-base font-black">
                                    <span class="text-slate-900">Net Grand Total</span>
                                    <span class="text-emerald-600">Rs. {{ number_format($this->dailySummaryData['grand_total'], 0) }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white border border-slate-100 rounded-[1.5rem] overflow-hidden shadow-sm">
                            <div class="px-6 py-5 border-b border-slate-50">
                                <h4 class="text-xs font-black text-slate-800 uppercase tracking-wider">Completed Orders Archive</h4>
                            </div>
                                        ...
                                        <tbody class="divide-y divide-slate-50 text-xs">
                                            @foreach($this->dailySummaryData['orders'] as $order)
                                                <tr class="hover:bg-slate-50/50 transition-colors">
                                                    <td class="px-6 py-4 font-bold text-slate-900 uppercase">#{{ substr($order->id, -6) }}</td>
                                                    <td class="px-6 py-4 text-slate-600 font-medium font-sans">
                                                        {{ $order->table ? ($order->table->type === 'room' ? 'Room ' : 'Table ') . $order->table->name : 'Takeaway' }}
                                                    </td>
                                                    <td class="px-6 py-4">
                                                        <span class="px-2 py-1 bg-slate-100 text-slate-700 font-bold rounded-lg uppercase text-[9px] tracking-wider">
    ...
    <div class="md:hidden fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-sm flex flex-col justify-end">
        <div class="bg-white rounded-t-3xl overflow-hidden flex flex-col max-h-[90vh]">
            <div class="p-4 border-b border-slate-100 flex justify-between items-center sticky top-0 bg-white">
                ...
                <button wire:click="closeOrderModal" class="p-2 bg-slate-100 rounded-full text-slate-500">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                </button>
            </div>
            
            <div class="p-6 overflow-y-auto">
                <div class="text-3xl font-black text-center text-slate-900 mb-6">
                ...
                <div class="space-y-3 mb-6">
                    @foreach($selectedOrder->items as $item)
                        <div class="flex justify-between text-sm">
                            ...
                            <span class="font-bold text-slate-800">Rs. {{ number_format($item->subtotal, 0) }}</span>
                        </div>
                    @endforeach
                </div>
                ...
                @else
                    <button wire:click="openPaymentModal" class="w-full py-4 bg-emerald-600 text-white font-bold rounded-xl shadow-lg shadow-emerald-600/20">
                        Take Payment
                    </button>
                @endif
            </div>
    ...
    <div class="fixed inset-0 z-[100] bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-white rounded-3xl overflow-hidden w-full max-w-lg shadow-2xl flex flex-col">
            <div class="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                <h3 class="font-bold text-xl text-slate-800">Complete Payment</h3>
            ...
            <div class="p-8">
                <div class="text-center mb-8">
                    <div class="text-sm text-slate-500 font-bold uppercase tracking-wider mb-2">Grand Total</div>
                    @php
                    ...
                    @endphp
                    <div class="text-5xl font-black text-emerald-600">Rs. {{ number_format($mTotal > 0 ? $mTotal : 0, 0) }}</div>
                </div>

                <div class="mb-8 p-6 bg-slate-50 rounded-2xl border border-slate-200">
                    <label class="block text-sm font-black text-slate-700 mb-3 uppercase tracking-widest">Apply Discount ({{ $restaurant->currency }})</label>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                            <span class="text-slate-400 font-bold">{{ $restaurant->currency }}</span>
                        </div>
                        <input type="number" wire:model.blur="discount" class="w-full pl-12 pr-4 py-4 bg-white border-2 border-emerald-100 rounded-xl text-xl font-black text-emerald-600 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 outline-none transition-all" placeholder="0.00">
                    </div>
                ...
                <div class="space-y-4" x-data="{ method: @entangle('paymentMethod') }">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <label class="cursor-pointer">
                            ...
                            <div :class="method === 'cash' ? 'border-emerald-500 bg-emerald-50 shadow-sm' : 'border-slate-200 hover:bg-slate-50'" class="p-4 rounded-xl border-2 transition-all flex items-center gap-3">
                        ...
                        <label class="cursor-pointer">
                            ...
                            <div :class="method === 'card' ? 'border-emerald-500 bg-emerald-50 shadow-sm' : 'border-slate-200 hover:bg-slate-50'" class="p-4 rounded-xl border-2 transition-all flex items-center gap-3">
                                <div class="w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-sm">💳</div>
                        ...
                        <label class="cursor-pointer col-span-2">
                            ...
                            <div :class="method === 'online' ? 'border-emerald-500 bg-emerald-50 shadow-sm' : 'border-slate-200 hover:bg-slate-50'" class="p-4 rounded-xl border-2 transition-all flex items-center gap-3">
                                <div class="w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-sm">📱</div>
                                <span class="font-bold text-slate-700">Online Wallet / Bank Transfer</span>
                            </div>
                        </label>
                        <label class="cursor-pointer col-span-2">
                            <input type="radio" x-model="method" value="split" class="sr-only">
                            <div :class="method === 'split' ? 'border-emerald-500 bg-emerald-50 shadow-sm' : 'border-slate-200 hover:bg-slate-50'" class="p-4 rounded-xl border-2 transition-all flex items-center gap-3">
                    ...
                    @if($paymentMethod === 'online')
                        <div class="mt-4 p-4 bg-slate-50 rounded-xl border border-slate-200">
                            ...
                            <select wire:model="paymentProvider" class="w-full px-4 py-3 rounded-lg border border-slate-300 focus:ring-emerald-500 focus:border-emerald-500">
                    ...
                    @if($paymentMethod === 'split')
                        <div class="mt-4 p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-4">
                            <div>
                                ...
                                <input type="number" wire:model.live.debounce.500ms="splitCashAmount" class="w-full px-4 py-3 rounded-lg border border-slate-300 focus:ring-emerald-500 focus:border-emerald-500 font-bold" placeholder="Enter Cash Paid">
                            </div>
                            <div class="flex justify-between items-center text-sm font-bold p-3 bg-emerald-50 border border-emerald-100 rounded-lg text-emerald-800">
            ...
            <div class="p-6 border-t border-slate-100 bg-slate-50 flex gap-3 justify-end">
                ...
                <button wire:click="processPayment" wire:loading.attr="disabled" class="px-8 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl shadow-lg shadow-emerald-600/20 active:scale-95 transition-all flex items-center gap-2 disabled:opacity-75 disabled:cursor-not-allowed">
