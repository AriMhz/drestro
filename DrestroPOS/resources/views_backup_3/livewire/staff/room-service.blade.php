<div x-data="{ showCart: false }" class="h-full flex flex-col overflow-hidden relative bg-[#f1f5f9]/30">
    ...
    @if(session()->has('success'))
        <div class="fixed top-20 right-8 z-[100] animate-in fade-in slide-in-from-right duration-500">
            <div class="bg-slate-900 text-white px-8 py-4 rounded-2xl shadow-2xl flex items-center gap-4 border border-slate-700">
                <div class="w-8 h-8 bg-emerald-500 rounded-full flex items-center justify-center text-white shadow-lg shadow-emerald-500/20">
    ...
    <div class="absolute inset-0 z-[60] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-8">
        <div class="max-w-md w-full bg-white rounded-[2.5rem] shadow-2xl border border-white overflow-hidden text-center animate-in zoom-in duration-300">
            ...
            <div class="p-10">
                <div class="bg-slate-50 rounded-2xl p-6 mb-8 border border-slate-100 text-center">
                    ...
                    <p class="text-slate-900 font-black text-2xl tracking-tight">{{ $currentOrder->order_number }}</p>
                </div>
    ...
    <div class="flex-1 flex flex-col overflow-y-auto custom-scrollbar">
        ...
        <div class="pt-8 md:pt-16 pb-6 md:pb-12 px-4 md:px-12 max-w-[1600px] mx-auto w-full flex flex-col md:flex-row items-start md:items-end justify-between gap-6 md:gap-10">
            <div>
                ...
                <h2 class="text-4xl md:text-6xl font-black text-slate-900 tracking-tighter leading-tight">Hotel Booking</h2>
            ...
            <div class="flex gap-4">
                <div class="flex items-center gap-4 px-6 md:px-8 py-3 md:py-4 bg-white rounded-xl md:rounded-2xl shadow-sm border border-slate-200">
                    <div class="flex items-center gap-2">
                        <span class="w-2 h-2 bg-emerald-500 rounded-full"></span>
                        <span class="text-[9px] font-bold text-slate-600 uppercase tracking-widest">Available</span>
                    </div>
                    <div class="w-px h-4 bg-slate-100"></div>
                    <div class="flex items-center gap-2">
                        <span class="w-2 h-2 bg-indigo-600 rounded-full"></span>
                        <span class="text-[9px] font-bold text-slate-600 uppercase tracking-widest">Occupied</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="max-w-[1600px] mx-auto w-full px-4 md:px-12 pb-20 grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-8">
            ...
            <button wire:click="selectTable('{{ $t->id }}')" 
                ...
                <div class="mt-6 md:mt-8 w-16 h-16 md:w-24 md:h-24 rounded-[1.25rem] md:rounded-[2.5rem] flex items-center justify-center transition-all duration-500 {{ $isOccupied ? 'bg-white/5 text-white border border-white/5' : 'bg-slate-50 text-slate-300 group-hover:bg-indigo-600 group-hover:text-white' }}">
                ...
                <div class="text-center w-full">
                    <span class="block text-3xl md:text-5xl font-black tracking-tighter mb-2 md:mb-4 {{ $isOccupied ? 'text-white' : 'text-slate-900 group-hover:text-indigo-600' }} transition-colors duration-500">{{ $t->name }}</span>
                    @if($isOccupied)
    ...
    <div class="flex-1 flex flex-col items-center justify-start bg-slate-50 p-4 md:p-12 pt-20 md:pt-32 overflow-y-auto custom-scrollbar">
        <div class="max-w-2xl w-full bg-white rounded-[2rem] md:rounded-[2.5rem] shadow-2xl border border-slate-200 flex flex-col mb-12">
            ...
            <div class="p-6 md:p-16 space-y-8 md:space-y-12">
                ...
                <div class="space-y-3 md:space-y-4">
                    ...
                    <div class="flex items-center bg-slate-50 border border-slate-200 rounded-2xl focus-within:bg-white focus-within:border-indigo-600 transition-all group overflow-hidden">
                        <div class="pl-4 md:pl-8 text-slate-300 group-focus-within:text-indigo-500 transition-colors">
                        ...
                        <input wire:model="guestName" type="text" placeholder="Full name of the primary guest" class="flex-1 bg-transparent border-none outline-none px-4 py-4 md:px-6 md:py-6 text-sm md:text-base text-slate-800 font-bold placeholder:text-slate-300">
                    </div>
                ...
                <div class="space-y-3 md:space-y-4">
                    ...
                    <div class="flex items-center bg-slate-50 border border-slate-200 rounded-2xl focus-within:bg-white focus-within:border-indigo-600 transition-all group overflow-hidden">
                        ...
                        <input wire:model="guestIdNumber" type="text" placeholder="ID or Citizenship Number" class="flex-1 bg-transparent border-none outline-none px-4 py-4 md:px-6 md:py-6 text-sm md:text-base text-slate-800 font-bold placeholder:text-slate-300">
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10">
                    ...
                    <div class="space-y-3 md:space-y-4">
                        ...
                        <div class="flex items-center bg-slate-50 border border-slate-200 rounded-2xl focus-within:bg-white focus-within:border-indigo-600 transition-all group overflow-hidden">
                            ...
                            <input wire:model.live="guestPhone" type="tel" maxlength="10" oninput="this.value = this.value.replace(/[^0-9]/g, '').slice(0, 10)" placeholder="98XXXXXXXX" class="flex-1 bg-transparent border-none outline-none px-4 py-4 md:px-6 md:py-6 text-sm md:text-base text-slate-800 font-bold placeholder:text-slate-300">
                        </div>
                    </div>
                    <!-- Tariff Input -->
                    <div class="space-y-3 md:space-y-4">
                        ...
                        <div class="flex items-center bg-slate-50 border border-slate-200 rounded-2xl focus-within:bg-white focus-within:border-indigo-600 transition-all group overflow-hidden">
                            <div class="pl-4 md:pl-8 text-slate-400 font-black group-focus-within:text-indigo-600 transition-colors text-sm md:text-base">
                                Rs.
                            </div>
                ...
                <div class="pt-4 md:pt-8 pb-2 md:pb-4">
                    <button wire:click="bookRoom" class="w-full py-5 md:py-7 bg-indigo-600 text-white rounded-[1.5rem] md:rounded-[2rem] font-bold shadow-xl md:shadow-2xl shadow-indigo-100 hover:bg-indigo-700 active:scale-95 transition-all uppercase tracking-[0.2em] md:tracking-[0.3em] text-[10px] md:text-[11px]">
                        Finalize Check-in & Open Stay
                    </button>
                    <button wire:click="cancelBookingMode" class="w-full mt-4 md:mt-6 py-3 md:py-4 bg-white text-slate-400 rounded-xl md:rounded-2xl font-bold text-[9px] md:text-[10px] uppercase tracking-widest hover:text-slate-600 transition-colors">
                        Cancel Registration
                    </button>
                </div>
            </div>
    ...
    <div class="flex flex-col lg:flex-row flex-1 overflow-hidden bg-white">
        <!-- Left Column: Menu Selector -->
        <div class="flex-1 flex flex-col bg-slate-50 overflow-hidden border-r border-slate-100">
            <!-- Menu Header -->
            <div class="bg-white px-8 py-6 border-b border-slate-200 flex justify-between items-center shadow-sm z-10">
                <div class="flex items-center gap-6">
                    <button wire:click="startNewOrder" class="p-3 bg-slate-50 text-slate-400 rounded-xl hover:bg-indigo-600 hover:text-white transition-all">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                    </button>
                    <div>
                        <h2 class="text-xl font-bold text-slate-900 leading-none">Room {{ \App\Models\Table::find($selectedTableId)?->name }}</h2>
            ...
            <!-- Category Filter Bar -->
            <div class="px-8 py-4 bg-white border-b border-slate-100 flex gap-3 overflow-x-auto hide-scrollbar scroll-smooth">
                <button wire:click="filterByCategory(null)" class="flex-shrink-0 px-6 py-2 {{ $activeCategoryId == null ? 'bg-indigo-600 text-white shadow-md' : 'bg-slate-100 text-slate-500 hover:bg-slate-200' }} rounded-xl text-[10px] font-bold uppercase tracking-wider transition-all">All Culinary</button>
                @foreach($categories as $category)
                <button wire:click="filterByCategory({{ $category->id }})" class="flex-shrink-0 px-6 py-2 {{ $activeCategoryId == $category->id ? 'bg-indigo-600 text-white shadow-md' : 'bg-slate-100 text-slate-500 hover:bg-slate-200' }} rounded-xl text-[10px] font-bold uppercase tracking-wider transition-all">{{ $category->name }}</button>
                @endforeach
            </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                    ...
                    <button wire:click="addToCart({{ $item->id }})" wire:loading.attr="disabled" wire:loading.class="opacity-75 scale-95 pointer-events-none" wire:target="addToCart({{ $item->id }})" class="group bg-white rounded-2xl overflow-hidden border border-slate-200 hover:shadow-xl hover:border-indigo-300 transition-all active:scale-95 text-left relative">
                        <div class="h-32 bg-slate-50 relative">
                            ...
                            @endif
                            <div class="absolute top-2 right-2 px-2 py-0.5 {{ $item->is_veg ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-red-50 text-red-600 border-red-100' }} rounded text-[8px] font-bold border">
                        ...
                        <div class="p-4">
                            <h4 class="text-[11px] font-bold text-slate-800 leading-tight h-8 line-clamp-2 mb-2 group-hover:text-indigo-600 transition-colors">{{ $item->name }}</h4>
        ...
        <div class="w-full lg:w-[400px] h-[40vh] lg:h-auto flex-shrink-0 bg-white flex flex-col shadow-[-20px_0_40px_rgba(0,0,0,0.02)] z-10">
            <div class="p-8 border-b border-slate-100 flex items-center justify-between">
                <div>
                    <h3 class="text-xl font-bold text-slate-900 tracking-tight">Stay Summary</h3>
            ...
            <div class="flex-1 overflow-y-auto p-8 space-y-5 custom-scrollbar bg-slate-50/30">
                ...
                <div class="p-5 bg-white rounded-2xl border border-slate-200 shadow-sm group hover:border-indigo-300 transition-all">
                    <div class="flex justify-between items-start mb-4">
                        <div class="flex-1 pr-4">
                            <h4 class="text-xs font-bold text-slate-800 leading-tight">{{ $item['name'] }}</h4>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center bg-slate-100 rounded-lg p-1">
                            <button wire:click="decrementQuantity({{ $index }})" wire:loading.attr="disabled" class="w-8 h-8 flex items-center justify-center text-slate-500 hover:bg-white hover:text-indigo-600 rounded-md transition-all disabled:opacity-50">-</button>
                            <span class="w-10 text-center text-xs font-bold text-slate-700">{{ $item['quantity'] }}</span>
                            <button wire:click="incrementQuantity({{ $index }})" wire:loading.attr="disabled" class="w-8 h-8 flex items-center justify-center text-slate-500 hover:bg-white hover:text-indigo-600 rounded-md transition-all disabled:opacity-50">+</button>
                        </div>
                        <span class="text-sm font-bold text-slate-900">Rs. {{ number_format($item['price'] * $item['quantity'], 0) }}</span>
                    </div>
                </div>
                @empty
            ...
            <!-- Footer Billing -->
            @if(!empty($cart))
            <div class="p-8 bg-white border-t border-slate-200 shadow-[0_-20px_40px_rgba(0,0,0,0.02)]">
                @if($orderError)
                <div class="flex items-center gap-2 p-3 bg-red-50 border border-red-200 rounded-xl mb-4">
                    ...
                    <span class="text-xs font-semibold text-red-700">{{ $orderError }}</span>
                </div>
                @endif
                <div class="flex justify-between items-center mb-8">
                    <span class="text-xs font-bold text-slate-400 uppercase tracking-widest">Order Total</span>
                    <span class="text-3xl font-black text-slate-900 tracking-tight">Rs. {{ number_format(collect($cart)->sum(fn($i) => $i['price'] * $i['quantity']), 0) }}</span>
                </div>
