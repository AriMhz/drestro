<div class="space-y-6" id="pdf-content">
    ...
    <div class="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 bg-slate-50 p-6 rounded-3xl border border-slate-200/60 shadow-sm print:bg-transparent print:border-none print:p-0 print:shadow-none">
        <div>
            <h1 class="text-2xl font-black text-slate-800 tracking-tight">Reports & Analytics</h1>
            <p class="text-xs font-bold text-slate-500 mt-1 print:hidden">Track your restaurant's performance across any period.</p>
            <p class="text-xs text-slate-400 font-black mt-2 bg-slate-100 px-3 py-1.5 rounded-xl border border-slate-200/60 inline-flex items-center gap-2 select-none print:bg-transparent print:border-none print:p-0 print:m-0">
        ...
        <div class="flex flex-col md:flex-row flex-wrap items-stretch gap-4 bg-white rounded-2xl border border-slate-200 p-4 shadow-sm print-hidden w-full xl:w-auto mt-2 xl:mt-0">
            <!-- Section 1: Presets -->
            <div class="flex flex-col gap-1.5 justify-between">
                ...
                <div class="flex items-center gap-1 bg-slate-100 p-1 rounded-xl h-[38px] shrink-0">
                    <button wire:click="setPeriod('today')"
                        class="h-full px-5 rounded-lg text-xs font-bold transition-all {{ $dateRange === 'today' ? 'bg-slate-900 text-white shadow-sm' : 'text-slate-600 hover:text-slate-900' }}">Today</button>
                    <button wire:click="setPeriod('week')"
                        class="h-full px-5 rounded-lg text-xs font-bold transition-all {{ $dateRange === 'week' ? 'bg-slate-900 text-white shadow-sm' : 'text-slate-600 hover:text-slate-900' }}">Week</button>
                    <button wire:click="setPeriod('month')"
                        class="h-full px-5 rounded-lg text-xs font-bold transition-all {{ $dateRange === 'month' ? 'bg-slate-900 text-white shadow-sm' : 'text-slate-600 hover:text-slate-900' }}">Month</button>
                </div>
            </div>
            ...
            <div wire:ignore wire:key="calendar-picker-reports" class="flex flex-col gap-1.5"
                 x-ref="pickerWrap"
                 x-data="{
                    pickerOpen: false,
            ...
            <div class="flex flex-col gap-1.5 justify-between">
                ...
                <div class="flex flex-wrap items-center gap-2 shrink-0">
                    <button onclick="window.print()" class="px-4 py-1.5 rounded-xl text-xs font-bold text-slate-700 bg-slate-100 hover:bg-slate-200 transition-all flex items-center gap-2 border border-slate-200 shadow-sm" title="Print Reports">
                        <svg class="w-3.5 h-3.5 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"></path></svg>
                        Print
                    </button>
                    <button onclick="downloadPDF('Reports.pdf')" class="px-4 py-1.5 rounded-xl text-xs font-bold text-indigo-700 bg-indigo-50 border border-indigo-100 hover:bg-indigo-100 transition-all flex items-center gap-2 shadow-sm" title="Save Reports as PDF">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm relative overflow-hidden group">
            ...
            <h3 class="text-sm font-bold text-slate-500 uppercase tracking-wider mb-2">Total Revenue</h3>
            <div class="text-4xl font-black text-slate-800">Rs. {{ number_format($stats['revenue'], 0) }}</div>
        </div>

        <div class="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm relative overflow-hidden group">
            <div class="absolute top-0 right-0 p-6 opacity-10 group-hover:opacity-20 group-hover:scale-110 transition-all">
                <svg class="w-16 h-16 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"></path></svg>
            </div>
            <h3 class="text-sm font-bold text-slate-500 uppercase tracking-wider mb-2">Total Orders</h3>
            <div class="text-4xl font-black text-slate-800">{{ $stats['orders'] }}</div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm relative overflow-hidden group">
            ...
            <h3 class="text-sm font-bold text-slate-500 uppercase tracking-wider mb-2">Average Order Value</h3>
            <div class="text-4xl font-black text-slate-800">Rs. {{ number_format($stats['aov'], 0) }}</div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col h-[400px]">
            <div class="p-6 border-b border-slate-100">
                <h3 class="font-bold text-lg text-slate-800">Top Selling Items</h3>
            </div>
            <div class="flex-1 overflow-y-auto p-4">
                <div class="space-y-4">
                    @forelse($topItems as $index => $item)
                        <div class="flex items-center gap-4">
                            <div class="w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm {{ $index == 0 ? 'bg-amber-100 text-amber-700' : ($index == 1 ? 'bg-slate-200 text-slate-600' : ($index == 2 ? 'bg-orange-100 text-orange-700' : 'bg-slate-100 text-slate-500')) }}">
                                #{{ $index + 1 }}
                            </div>
                            <div class="flex-1">
                                <h4 class="font-bold text-slate-800">{{ $item->menuItem->name }}</h4>
                                <div class="text-sm text-slate-500">{{ $item->total_quantity }} sold</div>
                            </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col h-[400px]">
            <div class="p-6 border-b border-slate-100">
                <h3 class="font-bold text-lg text-slate-800">Revenue by Payment Method</h3>
            </div>
            <div class="flex-1 overflow-y-auto p-4">
                <div class="space-y-4">
                    @forelse($paymentMethods as $method)
                        <div class="p-4 rounded-xl border border-slate-100 bg-slate-50 flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-full bg-white flex items-center justify-center shadow-sm text-xl border border-slate-200 print:hidden">
                                    {{ $method->payment_method === 'cash' ? '💵' : ($method->payment_method === 'card' ? '💳' : ($method->payment_method === 'split' ? '⚖️' : '📱')) }}
                                </div>
                                <div>
                                    <h4 class="font-bold text-slate-800 uppercase tracking-wide text-sm">{{ $method->payment_method === 'split' ? 'Split Payment' : $method->payment_method }}</h4>
                                    @if($method->payment_provider)
                                        <div class="text-xs font-bold text-blue-600">{{ ucfirst($method->payment_provider) }}</div>
                                    @endif
                                </div>
                            </div>
                            <div class="font-black text-slate-800 text-xl">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="px-6 py-5 border-b border-slate-100 flex justify-between items-center">
            <h3 class="text-lg font-bold text-slate-800">Staff Performance</h3>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/50">
                        <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Staff Member</th>
                        <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Role</th>
                        <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-center">Orders Handled</th>
                        <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-right">Total Sales</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 text-sm">
                    @forelse($this->staffPerformance as $perf)
                        <tr class="hover:bg-slate-50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-8 h-8 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center font-bold text-xs uppercase print:hidden">
                                        {{ substr($perf->user->name ?? 'U', 0, 1) }}
                                    </div>
                                    <span class="font-bold text-slate-800">{{ $perf->user->name ?? 'Unknown User' }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <span class="px-2 py-1 rounded-lg bg-slate-100 text-[10px] font-bold text-slate-500 uppercase tracking-wide">
