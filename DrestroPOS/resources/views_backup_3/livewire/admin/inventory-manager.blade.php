<div class="space-y-6" id="pdf-content">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 print-hidden">
        <div>
            ...
            <p class="text-sm text-slate-500 mt-1">Track raw materials, stock levels, and purchase costs.</p>
        </div>
        <div class="flex flex-wrap items-center gap-3 w-full sm:w-auto print-hidden">
            <button onclick="window.print()" class="flex-1 sm:flex-none px-4 py-2 bg-slate-100 text-slate-700 font-bold rounded-xl border border-slate-200 hover:bg-slate-200 transition-all flex items-center justify-center gap-2 whitespace-nowrap" title="Print Inventory">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"></path></svg>
                Print
            </button>
            <button onclick="downloadPDF('Inventory_Report.pdf')" class="flex-1 sm:flex-none px-4 py-2 bg-indigo-50 text-indigo-700 font-bold rounded-xl border border-indigo-200 hover:bg-indigo-100 transition-all flex items-center justify-center gap-2 whitespace-nowrap" title="Save Inventory as PDF">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-2xl font-black text-slate-800">{{ $this->items->count() }}</p>
            </div>
        </div>
        <div class="bg-white p-5 rounded-2xl border {{ $this->lowStockCount > 0 ? 'border-red-200 bg-red-50/30' : 'border-slate-100' }} shadow-sm flex items-center gap-4">
            <div class="w-12 h-12 rounded-xl {{ $this->lowStockCount > 0 ? 'bg-red-100' : 'bg-amber-50' }} flex items-center justify-center {{ $this->lowStockCount > 0 ? 'text-red-600' : 'text-amber-600' }}">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>
            </div>
            <div>
                <p class="text-sm text-slate-500 font-medium">Low Stock Alerts</p>
                <p class="text-2xl font-black {{ $this->lowStockCount > 0 ? 'text-red-600' : 'text-slate-800' }}">{{ $this->lowStockCount }}</p>
            </div>
        </div>
        <div class="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
            ...
            <div>
                <p class="text-sm text-slate-500 font-medium">Total Stock Value</p>
                <p class="text-2xl font-black text-emerald-600">Rs. {{ number_format($this->totalValue, 0) }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="flex flex-col sm:flex-row gap-3 sm:items-center print-hidden">
        <div class="relative flex-1 max-w-md">
            ...
            <input wire:model.live.debounce.300ms="searchQuery" type="text" placeholder="Search inventory..." class="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none">
        </div>
        <select wire:model.live="filterCategory" class="px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-medium focus:ring-2 focus:ring-emerald-500 outline-none">
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/80">
                        ...
                        <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-center">Stock</th>
                        <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-right">Cost/Unit</th>
                        <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-right">Value</th>
                        <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-center">Added Date</th>
                        <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-center">Status</th>
                        <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-right print-hidden">Actions</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 text-sm">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors {{ $item->isLowStock() ? 'bg-red-50/30' : '' }}">
                        ...
                        <td class="px-6 py-4">
                            ...
                            @endphp
                            <span class="px-2.5 py-1 rounded-lg bg-slate-100 text-xs font-semibold text-slate-600">
                                {{ $catIcons[$item->category] ?? '📋' }} {{ ucfirst($item->category) }}
                            </span>
                        </td>
                        <td class="px-6 py-4 text-center">
                            <span class="font-bold {{ $item->isLowStock() ? 'text-red-600' : 'text-slate-800' }}">{{ number_format($item->quantity, 1) }}</span>
                            <span class="text-slate-400 text-xs ml-1">{{ $item->unit }}</span>
                        </td>
                        <td class="px-6 py-4 text-right text-slate-600 font-medium">Rs. {{ number_format($item->cost_per_unit, 0) }}</td>
                        <td class="px-6 py-4 text-right font-bold text-slate-700">Rs. {{ number_format($item->quantity * $item->cost_per_unit, 0) }}</td>
                        <td class="px-6 py-4 text-center text-xs font-medium text-slate-500">{{ \App\Helpers\DateHelper::format($item->created_at) }}</td>
                        <td class="px-6 py-4 text-center">
                            @if($item->isLowStock())
                        ...
                        <td class="px-6 py-4 text-right print-hidden">
                            <div class="flex items-center justify-end gap-2">
                                <button wire:click="openHistoryModal({{ $item->id }})" class="px-2.5 py-1.5 bg-slate-100 text-slate-600 rounded-lg text-xs font-bold hover:bg-slate-200 transition-colors" title="View History">
                                    History
                                </button>
                                <button wire:click="openStockModal({{ $item->id }})" class="px-3 py-1.5 bg-blue-50 text-blue-600 rounded-lg text-xs font-bold hover:bg-blue-100 transition-colors" title="Adjust Stock">
                                    ± Stock
                                </button>
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-12 text-center">
                            ...
                            <p class="font-medium text-slate-500">No inventory items yet</p>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-xl max-w-lg w-full mx-4 overflow-hidden">
            <div class="px-6 py-5 border-b border-slate-100 flex justify-between items-center">
                <h3 class="text-lg font-bold text-slate-800">{{ $editingItemId ? 'Edit' : 'Add New' }} Inventory Item</h3>
            ...
            <div class="p-6 space-y-4">
                <div>
                    ...
                    <input wire:model.blur="itemName" type="text" class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none" placeholder="e.g. Chicken Breast">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select wire:model.blur="itemUnit" class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none">
                    ...
                    <div>
                        ...
                        <select wire:model.blur="itemCategory" class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none">
                            <option value="meat">🥩 Meat</option>
                            <option value="vegetables">🥬 Vegetables</option>
                            <option value="dairy">🧀 Dairy</option>
                            <option value="spices">🌶️ Spices</option>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">Cost per Unit (Rs.)</label>
                        <input wire:model.blur="itemCostPerUnit" type="number" step="0.01" min="0" class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none" placeholder="0">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">Low Stock Alert</label>
                        <input wire:model.blur="itemLowStockThreshold" type="number" step="0.1" min="0" class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none" placeholder="5">
                    </div>
                </div>
                @if(!$editingItemId)
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">Opening Stock</label>
                        <input wire:model.blur="itemQuantity" type="number" step="0.1" min="0" class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none" placeholder="0">
                        @error('itemQuantity') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">Stock Date</label>
                        <div wire:ignore wire:key="calendar-picker-item-date" class="relative animate-fade-in" 
                            ...
                            <div @click="pickerOpen = !pickerOpen" class="w-full flex items-center justify-between px-4 py-2.5 bg-white border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-50 transition-all select-none">
                                <span class="text-sm font-bold text-slate-700" x-text="getFormattedSelectedDate()"></span>
                            ...
                            <div x-show="pickerOpen" x-cloak 
                                 style="position: absolute; z-index: 9999; width: 320px; top: 100%; right: 0;"
                                 class="bg-white border border-slate-200 rounded-2xl shadow-2xl p-4 mt-2 space-y-3">
            ...
            <div class="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
                <button wire:click="closeItemModal" class="px-5 py-2.5 text-slate-600 font-semibold rounded-xl hover:bg-slate-200 transition-colors">Cancel</button>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-xl max-w-md w-full mx-4 overflow-hidden">
            <div class="px-6 py-5 border-b border-slate-100">
                ...
                <p class="text-sm text-slate-500 mt-1">{{ $stockItem->name ?? '' }} — Current: <strong>{{ number_format($stockItem->quantity ?? 0, 1) }} {{ $stockItem->unit ?? '' }}</strong></p>
            </div>
            <div class="p-6 space-y-4">
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Type</label>
                    <div class="grid grid-cols-3 gap-2">
                        <button wire:click="$set('stockType', 'stock_in')" class="py-2.5 rounded-xl text-sm font-bold text-center border-2 transition-all {{ $stockType == 'stock_in' ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-slate-200 text-slate-500' }}">
                            + Stock In
                        </button>
                        <button wire:click="$set('stockType', 'stock_out')" class="py-2.5 rounded-xl text-sm font-bold text-center border-2 transition-all {{ $stockType == 'stock_out' ? 'border-red-500 bg-red-50 text-red-700' : 'border-slate-200 text-slate-500' }}">
                            − Stock Out
                        </button>
                        <button wire:click="$set('stockType', 'adjustment')" class="py-2.5 rounded-xl text-sm font-bold text-center border-2 transition-all {{ $stockType == 'adjustment' ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-slate-200 text-slate-500' }}">
                            ✎ Set Exact
                        </button>
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">Quantity ({{ $stockItem->unit ?? '' }})</label>
                        <input wire:model.blur="stockQuantity" type="number" step="0.01" min="0.01" class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none text-lg font-bold" placeholder="0" autofocus>
                ...
                <div>
                    ...
                    <input wire:model.blur="stockNote" type="text" class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none" placeholder="e.g. Purchased from market or Sent to Kitchen">
                </div>
            </div>
            <div class="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
                <button wire:click="closeStockModal" class="px-5 py-2.5 text-slate-600 font-semibold rounded-xl hover:bg-slate-200 transition-colors">Cancel</button>
    ...
    @if($showHistoryModal)
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-xl max-w-2xl w-full mx-4 overflow-hidden flex flex-col max-h-[85vh]">
            <div class="px-6 py-5 border-b border-slate-100 flex justify-between items-center">
                <div>
                    <h3 class="text-lg font-bold text-slate-800">Stock History</h3>
                    <p class="text-sm text-slate-500 mt-1">{{ $historyItem->name ?? '' }}</p>
                </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead class="sticky top-0 bg-slate-100 border-b border-slate-200">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider text-right">Qty</th>
                            <th class="px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider text-right">Balance</th>
                            <th class="px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Note</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100 text-sm bg-white">
                        @forelse($historyLogs as $log)
                        <tr class="hover:bg-slate-50">
                            <td class="px-6 py-3 text-slate-600 whitespace-nowrap">{{ \App\Helpers\DateHelper::format($log->created_at) }} • {{ $log->created_at->format('h:i A') }}</td>
                            <td class="px-6 py-3">
                                @if($log->type === 'stock_in')
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-100 text-emerald-700 uppercase tracking-wide">Stock In</span>
                                @elseif($log->type === 'stock_out')
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-red-100 text-red-700 uppercase tracking-wide">Stock Out</span>
                                @else
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-blue-100 text-blue-700 uppercase tracking-wide">Adjustment</span>
                                @endif
                            </td>
                            <td class="px-6 py-3 text-right font-bold {{ $log->type === 'stock_in' ? 'text-emerald-600' : ($log->type === 'stock_out' ? 'text-red-600' : 'text-blue-600') }}">
                                {{ $log->type === 'stock_out' ? '-' : ($log->type === 'stock_in' ? '+' : '') }}{{ number_format($log->quantity, 1) }}
                            </td>
                            <td class="px-6 py-3 text-right font-medium text-slate-800">{{ number_format($log->balance_after, 1) }}</td>
                            <td class="px-6 py-3 text-slate-500 text-xs">{{ $log->note ?: '-' }}</td>
                        </tr>
                        @empty
                        <tr>
    ...
    <div class="mt-12 space-y-4 print-hidden">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <h2 class="text-xl font-bold text-slate-800">Daily Stock Movement</h2>
                <p class="text-sm text-slate-500">Track exactly what happened during a specific period.</p>
            </div>
            
            <div class="flex flex-wrap items-stretch gap-4 bg-white rounded-2xl border border-slate-200 p-4 shadow-sm w-full lg:w-auto">
                <!-- Section 1: Presets -->
                ...
                <div wire:ignore wire:key="calendar-picker-inventory" class="flex flex-col gap-1.5"
                     x-ref="pickerWrap"
                     x-data="{
                        pickerOpen: false,
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/50">
                            ...
                            <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Item</th>
                            <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Movement</th>
                            <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-right">Quantity</th>
                            <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-right">Final Balance</th>
                    ...
                    <tbody class="divide-y divide-slate-100 text-sm">
                        @forelse($this->dailyLogs as $log)
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                <td class="px-6 py-4 text-slate-500 font-medium">{{ $log->created_at->format('h:i A') }}</td>
                                <td class="px-6 py-4">
                                    <span class="font-bold text-slate-800">{{ $log->item->name ?? 'Deleted Item' }}</span>
                                    <span class="text-[10px] bg-slate-100 px-1.5 py-0.5 rounded ml-1 text-slate-500">{{ $log->item->unit ?? '' }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    @if($log->type === 'stock_in')
                                        <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-100 text-emerald-700 uppercase tracking-wide">Stock In</span>
                                ...
                                <td class="px-6 py-4 text-right font-bold {{ $log->type === 'stock_in' ? 'text-emerald-600' : ($log->type === 'stock_out' ? 'text-red-600' : 'text-blue-600') }}">
                                    {{ $log->type === 'stock_out' ? '-' : ($log->type === 'stock_in' ? '+' : '') }}{{ number_format($log->quantity, 1) }}
                                </td>
                                <td class="px-6 py-4 text-right font-bold text-slate-700">{{ number_format($log->balance_after, 1) }}</td>
                                <td class="px-6 py-4 text-slate-500 text-xs italic">{{ $log->note ?: 'No note' }}</td>
                            </tr>
                        @empty
                            <tr>
