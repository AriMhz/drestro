...
<style>
    ...
    @if($showBackupReminder)
        <div class="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm transition-opacity">
            <div class="bg-white rounded-3xl max-w-lg w-full shadow-2xl border border-slate-100 overflow-hidden transform transition-all duration-300 scale-100">
                <div class="relative p-6 sm:p-8 flex flex-col items-center text-center">
                    
                    <!-- Pulsing Warning/Backup Icon -->
                    <div class="w-16 h-16 rounded-full bg-amber-50 border border-amber-100 flex items-center justify-center text-amber-500 animate-bounce mb-4">
                        <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path>
                        </svg>
                    </div>

                    <h3 class="text-xl font-extrabold text-slate-800 tracking-tight">💾 Database Backup Reminder</h3>
                    ...
                    <div class="w-full mt-5">
                        @if (session()->has('success'))
                            <div class="p-3 bg-emerald-50 border border-emerald-100 text-emerald-800 text-xs rounded-xl font-bold text-left flex items-start gap-2">
                                <span>🎉</span> {{ session('success') }}
                            </div>
                        @endif
                        @if (session()->has('error'))
                            <div class="p-3 bg-rose-50 border border-rose-100 text-rose-800 text-xs rounded-xl font-bold text-left flex items-start gap-2">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full mt-6">
                        ...
                        <button type="button" wire:click="dismissBackupReminder"
                            class="px-5 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl active:scale-[0.98] transition-all">
                            Dismiss Reminder
                        </button>
                    </div>
        ...
        <div
            ...
            <div class="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <a href="/admin/license"
                    class="inline-flex items-center px-5 py-2.5 bg-white text-indigo-600 rounded-xl font-bold text-sm hover:bg-indigo-50 transition-colors shadow-sm">
                    Upgrade Now
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Analytics Overview</h1>
            <p class="text-sm text-slate-500 mt-1 print-hidden">Track your restaurant's real-time performance.</p>
            
            <!-- Subtitle including selected date range for printing only -->
            <div class="hidden print:block text-xs font-bold text-slate-600 mt-1 uppercase tracking-wider">
                @php
        ...
        <div class="flex flex-col md:flex-row flex-wrap items-stretch gap-4 bg-white rounded-2xl border border-slate-200 p-4 shadow-sm print-hidden w-full lg:w-auto mt-2">
            <!-- Section 1: Presets -->
                ...
                <div x-show="pickerOpen" x-cloak 
                    ...
                    <div class="pt-2 border-t border-slate-100 flex flex-col gap-1 text-[10px] select-none">
                        <div class="flex justify-between items-center text-[9px] text-slate-400 font-bold uppercase">
                            ...
                            <span class="text-emerald-600 font-black uppercase tracking-wider" x-text="pickingField === 'start' ? 'Selecting From Date' : 'Selecting To Date'"></span>
                        </div>
            ...
            <div class="flex flex-col gap-1.5 justify-between">
                ...
                <div class="flex flex-wrap items-center gap-2 shrink-0">
                    <button onclick="window.print()" class="px-4 py-1.5 rounded-xl text-xs font-bold text-slate-700 bg-slate-100 hover:bg-slate-200 transition-all flex items-center gap-2 border border-slate-200 shadow-sm" title="Print Dashboard">
                        <svg class="w-3.5 h-3.5 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"></path></svg>
                        Print
                    </button>
                    <button onclick="window.print()" class="px-4 py-1.5 rounded-xl text-xs font-bold text-indigo-700 bg-indigo-50 border border-indigo-100 hover:bg-indigo-100 transition-all flex items-center gap-2 shadow-sm" title="Save Dashboard as PDF">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div
            ...
            <div>
                ...
                <p class="text-3xl font-black text-slate-800 mt-1">{{ number_format($this->totalOrders) }}</p>
                <p class="text-xs text-slate-400 mt-2 capitalize">
                    {{ $period === 'today' ? "Today's orders" : ($period === 'week' ? 'This week' : 'This month') }}</p>
            </div>
            <div
                class="w-14 h-14 rounded-2xl bg-blue-50 flex items-center justify-center text-blue-600 group-hover:scale-110 transition-transform">
        ...
        <div
            ...
            <div>
                ...
                <p class="text-3xl font-black text-emerald-600 mt-1">Rs. {{ number_format($this->totalRevenue, 0) }}</p>
        ...
        <!-- Active Tables -->
        <a href="/staff/waiter"
            class="block bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between group hover:shadow-md hover:border-amber-200 transition-all cursor-pointer print-hidden">
            <div>
                <p class="text-sm font-medium text-slate-500 group-hover:text-amber-600 transition-colors">Active Tables
                    (Click to Manage)</p>
                <p class="text-3xl font-black text-slate-800 mt-1">{{ $this->activeTables['active'] }} <span
            ...
            <div
                class="w-14 h-14 rounded-2xl bg-amber-50 flex items-center justify-center text-amber-600 group-hover:bg-amber-100 group-hover:scale-110 transition-all">
                <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <!-- Pending Orders -->
        <div
            class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between group hover:shadow-md transition-shadow print-hidden">
            <div>
                <p class="text-sm font-medium text-slate-500">Pending Orders</p>
                <p
                    class="text-3xl font-black {{ $this->pendingOrders > 0 ? 'text-orange-500' : 'text-slate-800' }} mt-1">
                    {{ $this->pendingOrders }}</p>
                <p class="text-xs text-slate-400 mt-2">Awaiting kitchen / bar</p>
            </div>
            <div
                class="w-14 h-14 rounded-2xl bg-orange-50 flex items-center justify-center text-orange-500 group-hover:scale-110 transition-transform {{ $this->pendingOrders > 0 ? 'animate-pulse' : '' }}">
                <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden mt-6">
        <div class="px-6 py-5 border-b border-slate-100 flex justify-between items-center">
            ...
            <a href="/staff/cashier"
                class="text-sm font-semibold text-emerald-600 hover:text-emerald-700 transition-colors">View All →</a>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/50">
                        <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Order ID</th>
                        <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Table</th>
                ...
                <tbody class="divide-y divide-slate-100 text-sm">
                    @forelse($this->recentOrders as $order)
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-6 py-4 font-bold text-emerald-600">Rs.
                            ...
                            <td class="px-6 py-4">
                                ...
                                @endphp
                                <span
                                    class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold {{ $statusColors[$order->status] ?? 'bg-slate-100 text-slate-600' }}">
                                    {{ ucfirst($order->status) }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-slate-500">{{ $order->created_at->diffForHumans() }}</td>
                        </tr>
                    @empty
                        <tr>
