...
export default async function MarketingClientsPage() {
  ...
  // Query clients for marketing/sales staff (by creator OR assigned location)
  let clientWhere: any = undefined;
  if (staffRole === "MARKETING" || staffRole === "SALES") {
    if (staffLocation && staffLocation.trim().length > 0) {
      clientWhere = {
        OR: [
          { createdById: staffId },
          { location: { contains: staffLocation } }
        ]
      };
    } else {
      clientWhere = { createdById: staffId };
    }
  }
  ...
  // Query users for marketing/sales staff (by referral OR assignedLocation OR restaurant city/address)
    ...
    if (staffLocation && staffLocation.trim().length > 0) {
      userWhere = {
        OR: [
          ...
          { assignedLocation: { contains: staffLocation } },
          {
            restaurants: {
              some: {
                OR: [
                  { city: { contains: staffLocation } },
                  { address: { contains: staffLocation } }
                ]
              }
            }
          }
      ...
      userWhere = { referredById: staffId };
    }
  }
  ...
  async function generateLicenseKey(formData: FormData) {
    "use server";
    const session = await getSession();
    const creatorId = session?.staffId || null;
  ...
  async function updateSubscription(formData: FormData) {
    "use server";
    const session = await getSession();
    const updaterId = session?.staffId || null;
    ...
    await prisma.subscription.update({
      ...
      data: { planId, status, currentPeriodEnd: currentPeriodEnd ? new Date(currentPeriodEnd) : null, tableLimit, staffLimit, dishLimit, roomLimit, updatedById: updaterId }
    });
    revalidatePath("/marketing/clients");
  }
  ...
  async function resetMachineId(formData: FormData) {
    ...
    await prisma.client.update({ where: { id: clientId }, data: { machineId: null, updatedById: updaterId } });
    revalidatePath("/marketing/clients");
  }
